...
#endif /* IMU_ACC */
        {
            ...
            switch(IMAI_dequeue(label_scores))
            {
                ...
                case IMAI_RET_SUCCESS:
                {
                    ...
                    /* Build one complete frame and send it in a single UART
                     * write. Keep the generated unlabeled class for inference,
                     * but omit it from the user-facing score list. Hide the
                     * cursor while bytes are arriving, then park it at a fixed
                     * position so late serial connections also remain stable.
                     */
                    display_length += (size_t)snprintf(
                        &display_buffer[display_length],
                        sizeof(display_buffer) - display_length,
                        "\x1b[?25l\x1b[H");
                    ...
                    display_length += (size_t)snprintf(
                        &display_buffer[display_length],
                        sizeof(display_buffer) - display_length,
                        "DEEPCRAFT Studio Deploy Motion Example - CM33\x1b[K\r\n\x1b[K\r\n");
#else
                    display_length += (size_t)snprintf(
                        &display_buffer[display_length],
                        sizeof(display_buffer) - display_length,
                        "DEEPCRAFT Studio Deploy Motion Example - CM55\x1b[K\r\n\x1b[K\r\n");
#endif /* COMPONENT_CM33 */

                    for(int i = 0; i < IMAI_DATA_OUT_COUNT; i++)
                    {
                        if ((0 == strcmp(label_text[i], "unlabeled")) ||
                            (0 == strcmp(label_text[i], "unlabelled")))
                        {
                            continue;
                        }
                        ...
                        display_length += (size_t)snprintf(
                            &display_buffer[display_length],
                            sizeof(display_buffer) - display_length,
                            "label: %-10s: score: %f\x1b[K\r\n",
                            label_text[i], label_scores[i]);
                    }

                    const bool output_is_unlabeled =
                        (0 == strcmp(label_text[best_label], "unlabeled")) ||
                        (0 == strcmp(label_text[best_label], "unlabelled"));

                    display_length += (size_t)snprintf(
                        &display_buffer[display_length],
                        sizeof(display_buffer) - display_length,
                        "\x1b[K\r\nOutput: %-30s\x1b[K\r\n",
                        output_is_unlabeled ? "-" : label_text[best_label]);
#if defined(IMU_ACC) && defined(IMU_GYR)
                    display_length += (size_t)snprintf(
                        &display_buffer[display_length],
                        sizeof(display_buffer) - display_length,
                        "IMU: ax=% .4f ay=% .4f az=% .4f "
                        "gx=% .4f gy=% .4f gz=% .4f\x1b[K"
                        "\x1b[9;1H\x1b[?25h",
