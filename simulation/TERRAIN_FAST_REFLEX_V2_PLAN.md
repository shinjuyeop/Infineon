...
the only Slip positive went directly to Confirmed.

### Pilot-preceding target-policy freeze

The deployment policy is frozen before any v2 pilot, validation, or final
test: **Slip Hazard** is `confirmed_slip`; **Sand Sink Hazard** is
`sustained_sink`. `incipient_risk` is an oracle timing diagnostic only, not a
deployment target or readiness requirement. No threshold or label was changed
to fabricate a separate Incipient interval. `sustained_tilt` remains a
diagnostic/severity subgroup; isolated Sand Tilt-only is excluded because of
the completed bounded Digital Twin physical-design audit. Sink+Tilt remains a
Sand Sink positive.

Under this frozen target policy, the authoritative train-only calibration
passes the revised physical readiness gate: clean controls, confirmed Slip,
Sink, Sink+Tilt, valid loaded contact, native 1 kHz, and zero final-test
materialization. `PILOT_READY=true` applies only to this new policy artifact;
historical artifacts retain their original false result. The next user-run
pilot is four modes × five train/validation families × one realization × five
runs = 100 rows; it is not generated by this milestone.

### Final-scope pilot policy audit

The user-generated 100-row pilot was read-only audited under the frozen
confirmed-hazard policy. Its generation summary's `pilot_ready=false` was a
legacy `_selection` gate: it still required risk/tilt-oriented coverage and
counted diagnostic Tilt in normal. The shared frozen gate now evaluates only
confirmed Slip and sustained Sink targets, valid loaded contact, 2 kHz/1 kHz
sampling, split ownership, and sealed final test. It reports `PILOT_PASS=true`:
normal has 0 Confirmed Slip and 0 Sink; Slip has 19/25 Confirmed; Sink has
25/25 Sink; Sink+Tilt has 3/25 Sink. Historical pilot files remain unchanged;
the new policy report is
`outputs/terrain_fast_reflex_v2_final_scope_pilot_policy_audit`.

Target cross-coverage is retained rather than hidden: Sink-dominant has
10/25 Confirmed Slip and 13/25 Incipient diagnostics, while Slip has 0/25
Sink. Future detector endpoints must be assigned from their current oracle
target, never from mode names. Every family has at least one Confirmed Slip in
Slip mode (1/5, 5/5, 5/5, 5/5, 3/5 across the five families), and every family
has 5/5 Sink in Sink-dominant. Sink+Tilt is not retained as an independent full
mode: it has Sink in only 3/25 rows and no new target coverage, whereas
Sink-dominant already has 25/25 Sink and Tilt diagnostic in 22/25. Its average
FSR/accel/gyro values are also close (26.05/3.30/.00518 vs
25.23/3.37/.00606), so the target-centred full plan is normal, Slip, and
Sink-dominant: 3 × 5 × 3 × 5 = 225 rows. The full gate requires all valid,
